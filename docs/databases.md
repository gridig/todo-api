...
pgBackRest runs **co-located inside the TimescaleDB service container** (not as a separate sidecar), archiving WAL continuously and taking scheduled base backups. This is required because Railway gives one volume per service and does not share volumes across services, and because `archive_command` runs inside the Postgres process and needs the pgBackRest config + repo access in that same container. The backup repository lives in a **Railway Bucket** (Cloudflare R2 under the hood, S3-compatible API) — credentials stay inside Railway's secret store and egress is free. The implementation guide is [docs/pgbackrest-implementation.md](pgbackrest-implementation.md).

The config is generated by the container entrypoint from environment variables:

```ini
# /etc/pgbackrest/pgbackrest.conf (written by the entrypoint from env vars)
[global]
repo1-type=s3
...
repo1-retention-full=35        # 35 daily fulls → ~35-day PITR window
repo1-retention-diff=14        # diffs between fulls
repo1-retention-archive=35     # fulls' worth of WAL retained
start-fast=y
process-max=2
